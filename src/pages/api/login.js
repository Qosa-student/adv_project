...
export default async function handler(req, res) {
  ...
  try {
    ...
    const query = `SELECT u.id, u.name, u.email, u.password as password, r.name as role FROM users u LEFT JOIN roles r ON u.role_id = r.id WHERE u.email = ? LIMIT 1`;
    ...
    return res.status(200).json({ id: user.id, name: user.name, email: user.email, role: user.role || 'user' });
  } catch (err) {
    console.error(err);
