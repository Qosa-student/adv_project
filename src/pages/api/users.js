...
<<<<<<< HEAD
    ...
    if (req.method === 'DELETE') {
      ...
      if (!userId && !email) return res.status(400).json({ error: 'userId or email required to delete account' });
      ...
      try {
        ...
        if (!uid) {
          const [rows] = await conn.query('SELECT id FROM users WHERE email = ? LIMIT 1', [String(email).toLowerCase()]);
          if (!rows || rows.length === 0) {
            await conn.rollback();
            conn.release();
            return res.status(404).json({ error: 'User not found' });
        ...
        // delete dependent records (favorites, bookings, reviews) then user
        await conn.query('DELETE FROM favorites WHERE user_id = ?', [uid]).catch(() => {});
        await conn.query('DELETE FROM bookings WHERE user_id = ?', [uid]).catch(() => {});
        await conn.query('DELETE FROM reviews WHERE user_id = ?', [uid]).catch(() => {});
        await conn.query('DELETE FROM users WHERE id = ? LIMIT 1', [uid]);

        await conn.commit();
        conn.release();
        return res.status(200).json({ success: true, deletedUserId: uid });
      } catch (err) {
